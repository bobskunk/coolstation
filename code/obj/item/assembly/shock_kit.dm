...
/obj/item/assembly/shock_kit/receive_signal()
	if (src.master && istype(src.master, /obj/stool/chair/e_chair))
		...
		if (C.occupant)
			logTheThing("signalers", usr, C.occupant, "signalled an electric chair (setting: [C.lethal ? "lethal" : "non-lethal"]), shocking [constructTarget(C.occupant,"signalers")] at [log_loc(C)].") // Added (Convair880).
		C.shock()
	return
